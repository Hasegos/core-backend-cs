...
+ **Interface 기반:** Java에서 Queue는 인터페이스로 정의되어 있으며, 주로 `LinkedList`나 `ArrayDeque` 클래스로 구현하여 사용합니다.
...
+ **안정적인 메서드:** 큐가 비어있을 때 예외를 던지는 `remove()`보다, `null`을 반환하는 `poll()`을 사용하는 것이 안정적인 코드 작성에 유리합니다.

+ **활용 사례:** [queue.java](./queue.java)
